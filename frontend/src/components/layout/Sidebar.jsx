...
const NAV_MITR = [
  { to: '/screening',   icon: 'fa-search',         label: 'Screening' },
  { to: '/interviews',  icon: 'fa-star',           label: 'Interviews' },
  { to: '/selection',   icon: 'fa-check-double',   label: 'Final Selection' },
]
...
export default function Sidebar({ className = '', onNavigate, showCloseButton = false }) {
  ...
  const overflowClass = showCloseButton ? 'overflow-y-auto scrollbar-thin' : 'overflow-y-auto scrollbar-thin'
  ...
  return (
    <aside className={`w-[255px] xl:w-[275px] 2xl:w-[300px] bg-gradient-to-b from-maroon to-maroon-dark text-cream flex flex-col top-0 h-screen lg:sticky border-r-4 border-gold shrink-0 ${overflowClass} ${className}`}>
      {/* Brand */}
      <div className="flex items-center gap-3 px-4 xl:px-6 pt-5 xl:pt-7 pb-5 xl:pb-7 border-b border-white/20 mb-5 xl:mb-6 shrink-0">
      ...
      <nav className="px-4 xl:px-6 mb-4 xl:mb-5 shrink-0">
        <p className="text-[0.64rem] xl:text-[0.7rem] uppercase tracking-[0.15em] opacity-60 mb-2 xl:mb-3 font-semibold">Main Menu</p>
      ...
      {/* Modules Nav */}
      <nav className="px-4 xl:px-6 mb-4 xl:mb-5 shrink-0">
        <p className="text-[0.64rem] xl:text-[0.7rem] uppercase tracking-[0.15em] opacity-60 mb-2 xl:mb-3 font-semibold">Modules</p>
        {NAV_MODULES.map(({ to, icon, label }) => (
          <NavLink key={to} to={to} className={navLinkClass} onClick={onNavigate}>
            <i className={`fas ${icon} w-5 text-center text-gold`} />
            <span>{label}</span>
          </NavLink>
      ...
      {/* User Card */}
      <div className="mt-auto mx-4 xl:mx-6 mb-4 xl:mb-6 bg-white/5 border border-gold/20 rounded-2xl p-3.5 xl:p-5 flex items-center gap-3 shrink-0">
        <div
