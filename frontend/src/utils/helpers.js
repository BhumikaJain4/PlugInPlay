...
export function getTaskCountsByDay(tasks = [], month = 3, year = null) {
  return tasks.reduce((acc, t) => {
    const sameMonth = Number(t.due_month) === Number(month)
    const sameYear = year == null ? true : Number(t.due_year) === Number(year)
    if (t.status !== 'completed' && t.due_day >= 1 && t.due_day <= 31 && sameMonth && sameYear) {
