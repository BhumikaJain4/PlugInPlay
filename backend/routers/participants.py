...
async def import_from_sheet(body: ImportFromSheetRequest, current_user: User = Depends(get_current_user)):
    ...
    if response.status_code >= 400:
        raise HTTPException(status_code=400, detail="Could not download sheet CSV. Check sharing permissions.")

    text = response.text or ""
    if text.lstrip().lower().startswith("<!doctype html") or "<html" in text[:300].lower():
        raise HTTPException(status_code=400, detail="Sheet is not publicly readable as CSV. Share it with link access.")
...
async def auto_assign(body: AutoAssignRequest, current_user: User = Depends(get_current_user)):
    unassigned = [p for p in await ParticipantApplication.find_all().sort("+created_at").to_list()
                  if not p.screening_reviewer]
