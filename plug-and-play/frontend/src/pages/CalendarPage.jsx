...
export default function CalendarPage() {
  ...
  const dayTasks = selectedDay
    ? tasks.filter(t => t.due_day === selectedDay && t.due_month === 3)
    : []
    ...
    <div>
      <div className="mb-8 lg:mb-12">
        <div>
          ...
          <p className="text-base text-charcoal-light opacity-80 sm:text-lg">March 2026 schedule — click any day to see tasks</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 items-start gap-6 xl:grid-cols-[500px_minmax(0,1fr)] xl:gap-10">
        ...
        <div className="rounded-3xl border border-taupe-light bg-white p-4 sm:p-6 lg:p-8">
          <div className="flex justify-between items-center mb-6">
            <h2 className="font-serif text-2xl text-maroon">March 2026</h2>
            {selectedDay && (
          ...
          {isLoading ? <div className="flex justify-center py-8"><Spinner /></div>
            : <CalendarWidget tasks={tasks} onDayClick={d => setSelectedDay(prev => prev === d ? null : d)} selectedDay={selectedDay} />}
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-taupe-light pt-5 text-xs text-charcoal-light sm:gap-6">
            <span className="flex items-center gap-2"><span className="w-3 h-3 rounded-full bg-maroon inline-block" /> Today (Mar 20)</span>
          ...
          {selectedDay ? (
            <div>
              <h2 className="font-serif text-2xl text-maroon mb-6">
                March {selectedDay} — {dayTasks.length} task{dayTasks.length !== 1 ? 's' : ''}
              </h2>
              {dayTasks.length === 0 ? (
                <div className="rounded-3xl border border-taupe-light bg-white p-4 sm:p-6 lg:p-8">
